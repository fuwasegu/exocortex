...
| `evolved_from` | 進化した | 時間とともに判断が変化 |
| `rejected_because` | 却下された | ある理由でアプローチを却下 |
| `caused_by` | 引き起こされた | 過去の出来事が原因 |

**プロンプト例:**
- 「認可関連の記憶をリンクして」
- 「この知見は前のやつを発展させたものだからリンクして」
- 「この判断は前の判断から進化したから evolved_from でリンクして」

---

### 🕰️ 系譜の追跡 (`exo_trace_lineage`)

記憶の**進化と履歴**を追跡します。意思決定がどう変遷してきたかを理解できます。

| パラメータ | 説明 | 例 |
|-----------|------|-----|
| `memory_id` | 起点の記憶ID | 追跡を開始する記憶 |
| `direction` | `"backward"`（祖先）または `"forward"`（子孫） | `"backward"` |
| `relation_types` | 辿るリレーション | `["evolved_from", "caused_by"]` |
| `max_depth` | 最大探索深度 | `10`（デフォルト） |

**プロンプト例:**
- 「なんでこの判断になったの？履歴を追跡して」
- 「このバグの原因は何？系譜を見せて」
- 「このアーキテクチャはどう進化してきた？」

**ユースケース：**
- **アーキテクチャ考古学**: なぜ今こうなっているかを理解
- **根本原因分析**: 問題を起源まで追跡
- **意思決定の監査**: 重要な判断の変遷をレビュー

---

### 🤔 好奇心スキャン (`exo_curiosity_scan`)

知識ベースの矛盾・リンク候補・古い情報をスキャンし、質問を生成します。

| パラメータ | 説明 | 例 |
|-----------|------|-----|
| `context_filter` | プロジェクトで絞り込み | `"my-webapp"` |
| `tag_filter` | タグで絞り込み | `["architecture"]` |
| `max_findings` | カテゴリあたりの最大件数 | `10`（デフォルト） |

**検出する内容：**
- 🔴 **矛盾**: 同じトピックで成功 vs 失敗
- 🔗 **リンク候補**: リンクすべき未接続の記憶
- 📅 **古い情報**: supersededされていない古い知識
- ❓ **質問**: 知識に関する人間的な質問
...
| 戦略 | 説明 |
|------|------|
| **タグ共有** | 2つ以上のタグを共有する記憶（高信頼度） |
| **コンテキスト共有** | 同じプロジェクト＋同じタイプ（中信頼度） |
| **セマンティック類似度** | 70%以上の類似度（高信頼度） |
...
レスポンスには `next_actions` が含まれ、`exo_link_memories` の呼び出しが提案されます：
...
1. exo_curiosity_scan で矛盾・リンク候補・古い情報を検出
2. 生成された質問を確認
3. next_actions を実行してリンク候補を接続
4. 矛盾する記憶を evolved_from や supersedes でリンク
5. 古い記憶を superseded としてマーク
```
...
1. exo_curiosity_scan → suggested_links を取得
2. AI が next_actions を実行（exo_link_memories 呼び出し）
3. 知識グラフがより豊かに、より繋がりのあるものに！
```
...
├─ 「キャッシュはうまくいった」（成功）
└─ 「キャッシュは失敗した」（失敗）
    ↳ 学習の過程なら evolved_from でリンク
    ↳ 置き換えなら supersedes でリンク
```
...
1. exo_trace_lineage(direction="backward") で祖先を探す
2. evolved_from や caused_by の連鎖を辿る
3. 判断の全履歴を理解
```
...
現在: マイクロサービスアーキテクチャ
    ...
    │
    ├─ [depth 1] 「モノリスから移行」 (evolved_from)
    │
    └─ [depth 2] 「DBスケーリング問題」 (caused_by)
           ↳ 根本原因を特定！
```
...
- **判断の進化** → `evolved_from` でリンクして判断の変遷を追跡
- **根本原因** → `caused_by` でリンクして結果と原因を接続
- **却下されたアプローチ** → `rejected_because` でリンクして却下理由を記録
